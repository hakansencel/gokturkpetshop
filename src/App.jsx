    ...
    <div className="min-h-screen bg-[#070814] text-slate-100 antialiased">
        ...
        <div className="px-4 md:px-8 max-w-6xl mx-auto flex flex-col md:flex-row items-center gap-12">
          <div className="flex-1 text-center md:text-left">
            ...
            <div className="flex flex-col sm:flex-row justify-center md:justify-start gap-4">
              <a href={whUrl} target="_blank" rel="noreferrer"><button className="rounded-2xl bg-cyan-500 text-slate-950 font-bold px-8 py-3 text-sm flex items-center gap-2 justify-center"><Phone className="w-4 h-4 fill-current"/> İletişime Geçin</button></a>
      ...
      <div className="px-4 md:px-8 max-w-6xl mx-auto py-24 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {categories.map((cat, i) => {
          const Icon = cat.icon;
          return (
            <div key={i} className="rounded-3xl border border-white/10 bg-white/5 p-8 flex flex-col justify-between cursor-pointer hover:border-white/20" onClick={() => { setActivePage(cat.id); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
              <div>
      ...
      <div className="px-4 md:px-8 max-w-6xl mx-auto py-24 grid gap-12 md:grid-cols-2 items-center">
        ...
        <div className="h-80 w-full bg-slate-900 border border-white/10 rounded-3xl overflow-hidden relative shadow-2xl"><iframe title="Harita" src="https://google.com" width="100%" height="100%" style={{ border: 0 }} allowFullScreen="" loading="lazy" /></div>
      </div>
