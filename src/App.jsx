  ...
  return (
    <div className="min-h-screen bg-[#f8fafc] text-slate-700 antialiased selection:bg-cyan-500 selection:text-white">
      {showPopup && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-[200] flex items-center justify-center p-4 overflow-y-auto">
          ...
          <div className="relative z-10 my-auto animate-in fade-in zoom-in-95 duration-300">
            <img src="/popup-image.jpg" alt="Özel Fırsat" className="max-w-full max-h-[85vh] h-auto w-auto rounded-3xl shadow-2xl border-4 border-white object-contain" onError={() => setShowPopup(false)} />
      ...
      <div className="px-4 md:px-8 max-w-6xl mx-auto py-20 grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
        {cats.map((cat, i) => (
          <div key={i} className="rounded-3xl border border-slate-200/60 bg-white p-5 flex flex-col justify-between cursor-pointer hover:border-cyan-500/40 hover:shadow-xl transition-all duration-300 group shadow-sm" onClick={() => { setPage(cat.id); window.scrollTo({ top: 0, behavior: "smooth" }); }}>
            <div>
      ...
      <div className="px-4 md:px-8 max-w-6xl mx-auto py-24 grid gap-12 md:grid-cols-2 items-center">
        <div>
          ...
          <div className="space-y-5">
            <div className="flex gap-4 items-start">
              ...
              <div><h4 className="font-bold text-slate-800 mb-0.5">Adres</h4><p className="text-sm text-slate-500">Göktürk Merkez Mh. Göktürk Cd. No: 42, Eyüpsultan / İstanbul</p></div>
            </div>
            <div className="flex gap-4 items-start">
        ...
        <div className="h-80 w-full bg-slate-50 border border-slate-200 rounded-3xl overflow-hidden relative shadow-lg">
          <a href="https://maps.google.com" target="_blank" rel="noopener noreferrer" className="w-full h-full bg-slate-50 flex flex-col items-center justify-center p-6 text-center gap-4 group hover:bg-slate-100/70 transition">
